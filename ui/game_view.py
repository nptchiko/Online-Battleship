...
class GameView(tk.Tk):
    ...
    def _setup_callbacks(self):
        ...
        self.control_panel.set_random_placement_callback(
            self._random_placement)
        self.control_panel.set_orientation_change_callback(
    ...
    def _update_ui(self):
        ...
        # Update board views
        self.player_board_view.update_view(
            self.game_state.get_player_board(), show_ships=True)
        self.ai_board_view.update_view(
            self.game_state.get_ai_board(), show_ships=False)

        # Update control panel
        self.control_panel.update_status(self.game_state.get_message())
        self.control_panel.update_game_phase(
            self.game_state.get_current_phase())
    ...
    def _on_player_board_click(self, row, col):
        ...
        if self.game_state.get_current_phase() == GamePhase.SETUP:
            ...
            if self._can_place_ship(row, col, self.current_ship_size, orientation):
                success = self.game_state.place_player_ship(
                    row, col, orientation)
                ...
                if self.game_state.get_current_phase() == GamePhase.PLAYER_TURN:
                    messagebox.showinfo(
                        "Ships Placed", "All ships placed. Your turn to fire!")
            else:
                # Show error message for invalid placement
                self.control_panel.update_status(
                    "Invalid ship placement! Ships must be within the board and not overlap with other ships.")
    ...
    def _on_player_board_hover(self, row, col):
        ...
        if self.game_state.get_current_phase() == GamePhase.SETUP and self.current_ship_size:
            ...
            valid = self._can_place_ship(
                row, col, self.current_ship_size, orientation)
    ...
    def _on_ai_board_click(self, row, col):
        ...
        if self.game_state.get_current_phase() == GamePhase.PLAYER_TURN:
            ...
            if cell and cell.is_attacked():
                ...
                self.control_panel.update_status(
                    "You've already fired at this location. Try another spot.")
                return

            # Player's turn to fire
            valid_move, hit, ship_sunk, game_over = self.game_state.player_shoot(
                row, col)
    ...
    def _ai_turn(self):
        ...
        if self.game_state.get_current_phase() == GamePhase.AI_TURN:
            # Get the AI's move
            row, col = self.ai_strategy.get_move(
                self.game_state.get_player_board())
            ...
            if row is not None and col is not None:
                ...
                self.ai_strategy.notify_result(
                    row, col, hit, ship_sunk, game_over)
    ...
    def _random_placement(self):
        ...
        if success:
            ...
            if self.game_state.get_current_phase() == GamePhase.PLAYER_TURN:
                messagebox.showinfo(
                    "Ships Placed", "All ships placed randomly. Your turn to fire!")
        else:
            messagebox.showerror(
                "Error", "Failed to place ships randomly. Please try again.")
